...
class PCGrad:
    ...
    def _project_conflicting(self, grads, has_grads, shapes=None):
        ...
        for g_i in pc_grad:
            # shuffle *in place* copy to avoid bias
            random.shuffle(grads)
            for g_j in grads:
                # Cast to float64 to avoid overflow on very long vectors
                g_i_g_j = torch.vdot(g_i.double(), g_j.double())
                if g_i_g_j < 0:
                    denom = g_j.double().norm() ** 2 + 1e-16
                    g_i -= (g_i_g_j / denom).to(g_i.dtype) * g_j
        ...
        for group in self._optim.param_groups:
            for p in group["params"]:
                if not p.requires_grad:
                    # skip frozen parameter but keep index aligned
                    idx += 1
                    continue
                assert (
                    idx < len(grads)
                ), "Gradient list shorter than parameter list – packing mismatch."
                p.grad = grads[idx]
                idx += 1
        return
    ...
    def _pack_grad(self, objectives):
        ...
        for obj in objectives:
            ...
            # free graph on last objective
            retain_flag = obj is not objectives[-1]
            obj.backward(retain_graph=retain_flag)
